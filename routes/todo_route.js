...
router.post("/", async (req, res) => {
  ...
  await todo
    ...
    .then(() => {
      res
        .status(201)
        .json({ todo: todo, response_message: "Successful response" });
    })
    .catch(() => {
...
router.get("/", async (req, res) => {
  try {
    ...
    res
      .status(200)
      .json({ todo: todos, response_message: "Successful response" });
  } catch {
    res.status(452).send({ response_message: "Something went wrong" });
  }
});
...
router.get("/:id", async (req, res) => {
  try {
    ...
    if (!todo)
      return res
        .status(200)
        .json({ todo: null, response_message: "Successful response" });
